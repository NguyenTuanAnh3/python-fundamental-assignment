...
from config.constants import BILL_DIR

class ExcelHandler:
    def __init__(self, car_identity, leave_day, bills):
        self.car_identity = car_identity
        self.leave_day = leave_day
        self.bills = bills

    def create_excel(self):
        path = f'{BILL_DIR}{self.car_identity}.xlsx'
        leave_day = datetime.strptime(self.leave_day,"%Y-%m-%d %H:%M").strftime("%Y-%m-%d")
        ...
        self.format_excel(ws=ws)
        
        wb.save(path)

    def format_excel(self, ws):
        num_col = len([bill['date'] for bill in self.bills])
        self.create_title_yaxis(ws=ws)
        self.create_title_xaxis(ws=ws, num_col = num_col)
        self.filled_data(ws=ws, num_col = num_col)

    def create_title_yaxis(self, ws):
    ...
    def create_title_xaxis(self, ws, num_col):
        end_col = num_col * 2 + 1
        get_date = [bill['date'] for bill in self.bills]
        get_day_of_week = [bill['dayOfWeek'] for bill in self.bills]
        count_date = 0
        count_day_of_week = 0
    ...
    def filled_data(self, ws, num_col):
        end_col = num_col * 2 + 2
        get_data_day_time = [ bill['08:00 - 16:59'] for bill in self.bills ]
        get_data_night_time = [ bill['17:00 - 23:59'] for bill in self.bills ]
        get_data_over_night = [ bill['00:00 - 07:59'] for bill in self.bills ]
        get_data_sub_total = [bill['sub_total'] for bill in self.bills]
